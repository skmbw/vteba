...
public class BindGen
{
	...
	public BindGen(GlobalCustom glob) {
        m_global = glob;
        m_includeSet = new HashSet<String>();
        m_ignoreSet = new HashSet<String>();
        m_directSet = new HashSet<String>();
        m_superSet = new HashSet<String>();
        m_formatSet = new HashSet<String>();
        m_mappingDetailsMap = new HashMap<String, BindingMappingDetail>();
        m_typeNamesMap = new HashMap<String, UniqueNameSet>();
        m_elementNamesMap = new HashMap<String, UniqueNameSet>();
    ...
    public void expandReferences(String type, ReferenceCountMap refmap) {
        if (checkInclude(type)) {
            ...
            for (Iterator iter = clas.getMembers().iterator(); iter.hasNext();) {
                ...
                String mtype = memb.isCollection() ? memb.getItemType() : memb.getWorkingType();
                if (!Types.isSimpleValue(mtype) && !isValueClass(mtype) && !m_global.isKnownMapping(mtype)) {
                    if ((mtype.startsWith("java.")) || (mtype.startsWith("javax."))) {//兼容map
                    	if (mtype.equals("java.util.Map")) {
                    		String fieldName = memb.getFieldName();
                    		IClassItem classItem = clas.getClassInformation().getField(fieldName);
                    		String signature = classItem.getGenericsSignature();//Signature(Ljava/util/Map<Ljava/lang/String;Lcom/hch/testjibx/Address;>;)
                    		signature = signature.substring(signature.indexOf(";") + 2, signature.length() - 4).replaceAll("/", ".");
                    		mtype = signature;
                    		if (refmap.incrementCount(mtype) <= 1) {
                                expandReferences(mtype, refmap);
                            }
                            m_directSet.add(mtype);
                            m_superSet.add(mtype);//对于Map中包含的实体，需要映射具体的，然后才能绑定编译
                    	} else {
    ...
    private void setTypes(ValueCustom memb, StructureElementBase struct) {
        ...
        if (memb.getStatedType().equals("java.util.Map")) {
        	struct.setMarshallerName("org.jibx.extras.HashMapperStringToComplex");
        	struct.setUnmarshallerName("org.jibx.extras.HashMapperStringToComplex");
        }
     ...
     */
    private void addMemberBindings(ClassCustom cust, Map<String, ValueCustom> exmethmap, Map<String, ValueCustom> inmethmap, NestingElementBase parent,
        BindingHolder hold) {
     ...
     */
    private boolean isQNameUsed(QName qname, Map<String, UniqueNameSet> map) {
        if (qname != null) {
            UniqueNameSet nameset = map.get(qname.getUri());
            if (nameset != null) {
     ...
     */
    private QName fixQName(QName qname, Map<String, UniqueNameSet> map) {
        if (qname != null) {
            String uri = qname.getUri();
     ...
     * @param refmap
     */
    private void findReferences(List<String> classes, ReferenceCountMap refmap) {
    ...
    private void addReferencedMappings(ReferenceCountMap refmap) {
        for (Iterator<String> iter = refmap.iterator(); iter.hasNext();) {
            String type = (String)iter.next();
     ...
     * @param classes
     */
    private void generateMappings(List<String> classes) {
        for (int i = 0; i < classes.size(); i++) {
            String type = classes.get(i);
            BindingMappingDetail detail = (BindingMappingDetail)m_mappingDetailsMap.get(type);
            if (!detail.isGenerated()) {
    ...
    private void fixBaseClasses() {
        for (Iterator<String> iter = m_directSet.iterator(); iter.hasNext();) {
    ...
    private void generateFormats() {
        ...
        for (Iterator<String> iter = m_formatSet.iterator(); iter.hasNext();) {
            String type = iter.next();
     ...
     */
    public void generate(Boolean abstr, List<String> classes) {
     ...
     */
    public void generateSpecified(ArrayList<QName> qnames, List<String> concrs, List<String> abstrs) {
     ...
     */
    public List<BindingElement> validateFiles(File dir, IClassLocator loc, BindingHolder root) throws IOException, JiBXException {
...
//        ArrayList<ValidationProblem> list = vctx.getProblems();
//        for (ValidationProblem object : list) {
//        	Object com = object.getComponent();
//        	if (object.getDescription().indexOf("java.util.Map") >= 0) {
//        		
//        	}
//        }
        if (binding == null || vctx.getErrorCount() > 0 || vctx.getFatalCount() > 0) {
            ...
            List<String> uris = m_directory.getKeys();
            List<BindingElement> bindings = new ArrayList<BindingElement>();
    ...
    public static void main(String[] args) throws JiBXException, IOException {
        ...
        if (args.length > 0 && parms.processArgs(args)) {
            ...
            List<BindingElement> bindings = gen.validateFiles(parms.getGeneratePath(), parms.getLocator(), root);
            if (!parms.isBindingOnly()) {
                ...
                List<BindingElement> schemas = sgen.generate(bindings);
    ...
    public static String[] parseClass() throws IOException, DocumentException {
    	List<String> list = new LinkedList<String>();
    	list.add("-o");
    	list.add("-t");
    	list.add("./src/main/config");
    	list.add("-b");
    	list.add("Binding.xml");
    	
    	String filename = "/home/yinlei/downloads/vteba/src/main/resources/spring/application-oxm-jibx.xml";
    	...
    	for (Element ele : elementList) {
    		String text = ele.attributeValue("class");
    		if (text.equals(JibxMarshallerFactory.class.getName())) {
    			List<Element> eleList = ele.elements("property");
        		for (Element element : eleList) {
        			String name = element.attributeValue("name");
        			if (name.equals("targetClassList")) {
        				Element elist = element.element("list");
        				List<Element> bbElements = elist.elements("value");
        				for (Element cc : bbElements) {
        					list.add(cc.getTextTrim());
        				}
        			}
        		}
    		}
