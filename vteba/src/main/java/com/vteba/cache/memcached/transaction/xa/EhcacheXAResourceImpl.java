...
import com.vteba.cache.memcached.transaction.TransactionIDNotFoundException;
import com.vteba.cache.memcached.transaction.manager.TransactionManagerLookup;
import com.vteba.cache.memcached.transaction.xa.commands.Command;
import com.vteba.cache.memcached.transaction.xa.processor.XARequest;
import com.vteba.cache.memcached.transaction.xa.processor.XARequestProcessor;
...
public class EhcacheXAResourceImpl implements EhcacheXAResource {
     ...
     */
    public EhcacheXAResourceImpl(Memcache cache, Store underlyingStore, TransactionManagerLookup txnManagerLookup,
        ...
        this.comparator = new DefaultElementValueComparator();//cache.getElementValueComparator();
    }
    ...
    public void commitInternal(Xid xid, boolean onePhase) throws XAException {
        ...
        try {
            ...
            for (SoftLock softLock : softLocks) {
                Element e = underlyingStore.get(softLock.getKey());
                if (e == null) {
...
//                }
    ...
    public Xid[] recover(int flags) throws XAException {
        ...
        Thread t = new Thread("memcache [" + cache.getName() + "] XA recovery thread") {
            @Override
            public void run() {
