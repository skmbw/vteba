...
/**
 * Superclass for all Memcache specific read/write AccessStrategy implementations。
 * <br/>特定于Memcache的所有读写访问策略实现的公共超类。
 * @param <T> the type of the enclosed cache region
 */
@SuppressWarnings({"rawtypes","unchecked"})
public abstract class AbstractReadWriteMemcacheAccessStrategy<T extends MemcacheTransactionalDataRegion>
        extends AbstractMemcacheAccessStrategy<T> {

    private static final Logger LOG = LoggerFactory.getLogger(AbstractReadWriteMemcacheAccessStrategy.class);
     ...
     * 创建一个读/写缓存的访问策略，围绕/访问给定的缓存区域
     */
    public AbstractReadWriteMemcacheAccessStrategy(T region, Settings settings) {
        super( region, settings );
        this.versionComparator = region.getCacheDataDescription().getVersionComparator();
    }

    /**
     * 如果item是不可读的，返回<code>null</code>。  Locked items are not readable, nor are items created
     ...
     * key。
     * 对于给定的key，如果已经存在一个不可写的对象，返回false。否则将对象存入缓存
    ...
    protected void handleLockExpiry(Object key, Lockable lock) {
        LOG.info("softLockedCacheExpired" + region.getName(), key, lock == null ? "(null)" : lock.toString() );
    ...
    protected static interface Lockable {
         ...
         * 如果被封装的值，在事务开始的时间，可以被指定的版本取代，返回true
         */
