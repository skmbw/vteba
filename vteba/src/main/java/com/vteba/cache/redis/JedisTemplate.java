...
public class JedisTemplate {
	...
	public void psetex(final String key, final int seconds, final String value) {
		execute(new JedisNoResultAction() {
			...
			@Override
			public void action(Jedis jedis) {
				jedis.psetex(key, seconds * 1000, value);
			}
		});
	}
	...
	public String set(final String key, final Object object) {
		return execute(new JedisAction<String>() {
			...
			@Override
			public String action(Jedis jedis) {
				return jedis.set(key.getBytes(Char.UTF8), Kryos.get().toBytes(object));
			}
		});
	}
	
	/**
	 * 以key为键缓存object，seconds秒后过期。
	 * @param key 缓存键
	 * @param object 缓存值
	 * @param seconds 过期时间，单位秒
	 * @return 成功时OK，错误时返回状态码
	 */
	public String setex(final String key, final Object object, final int seconds) {
		return execute(new JedisAction<String>() {

			@Override
			public String action(Jedis jedis) {
				return jedis.setex(key.getBytes(Char.UTF8), seconds, Kryos.get().toBytes(object));
			}
		});
	}
	
	/**
	 * 以字段field缓存对象object到hash（key）中。
	 * @param key hash
	...
	public Long hset(final String key, final String field, final Object object) {
		return execute(new JedisAction<Long>() {
			...
			@Override
			public Long action(Jedis jedis) {
				return jedis.hset(key.getBytes(Char.UTF8), field.getBytes(Char.UTF8), Kryos.get().toBytes(object));
			}
		});
	}
	 ...
	 * @return OK
	 */
	public String hmset(final Map<String, Object> map, final String key) {
		return execute(new JedisAction<String>() {
			...
			public String action(Jedis jedis) {
				...
				for (Entry<String, Object> entry : map.entrySet()) {
					hash.put(entry.getKey().getBytes(Char.UTF8), Kryos.get().toBytes(entry.getValue()));
				}
	...
	public <T> T get(final String key, final Class<T> clazz) {
		return execute(new JedisAction<T>() {
			...
			public T action(Jedis jedis) {
				...
				return Kryos.get().fromBytes(bytes, clazz);
			}
		});
	}
	
	/**
	 * 批量获取key对应的缓存对象，返回List&ltT&gt
	 * @param clazz 返回对象类型
	 * @param keys 多个key，String[]
	 * @return 返回List&ltT&gt
	 */
	public <T> List<T> mget(final Class<T> clazz, final String... keys) {
		return execute(new JedisAction<List<T>>() {
			...
			public List<T> action(Jedis jedis) {
				byte[][] keyBytes = ByteUtils.toBytes(keys);
				List<byte[]> list = jedis.mget(keyBytes);
				List<T> rstList = new ArrayList<T>();
				for (byte[] bytes : list) {
					T t = Kryos.get().fromBytes(bytes, clazz);
					rstList.add(t);
				}
				return rstList;
			}
	 ...
	 * @return 对象List
	 */
	public <T> List<T> hgetAll(final String key, final Class<T> clazz) {
		return execute(new JedisAction<List<T>>() {
			...
			public List<T> action(Jedis jedis) {
				...
				for (Entry<byte[], byte[]> entry : map.entrySet()) {
					T t = Kryos.get().fromBytes(entry.getValue(), clazz);
					rstList.add(t);
				}
				return rstList;
			}
	...
	public Long sadd(final String key, final Object... members) {
		return execute(new JedisAction<Long>() {
			...
			@Override
			public Long action(Jedis jedis) {
				byte[][] bytes = ByteUtils.toBytes(members);
				return jedis.sadd(key.getBytes(Char.UTF8), bytes);
			}
		});
	}
	 ...
	 * @return 对象List
	 */
	public <T> List<T> smembers(final String key, final Class<T> clazz) {
		return execute(new JedisAction<List<T>>() {
